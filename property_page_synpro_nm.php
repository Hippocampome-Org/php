...
{	
	...
	$creatable=	"CREATE TABLE IF NOT EXISTS `$name_temporary_table` (
				   ...
				   issue varchar(20),
				   PRIMARY KEY (id));";
				   //echo "<br><br><br><br><br><br><br>".$creatable;
...
if (isset($_REQUEST['id1_neuron'])) {$id1_neuron=$_REQUEST['id1_neuron'];}
if (isset($_REQUEST['id2_neuron'])) {$id2_neuron=$_REQUEST['id2_neuron'];}
if (isset($_REQUEST['connection_type'])) {$connection_type=$_REQUEST['connection_type'];}
if (isset($_REQUEST['known_conn_flag'])) {$known_conn_flag=$_REQUEST['known_conn_flag'];}
...
$conn_val_property[0] = $fragment -> neuron_to_subregion($conn_id_neuron[0]);
$conn_val_property[1] = $fragment -> neuron_to_subregion($conn_id_neuron[1]);
      ...
      All of the evidence provided on Hippocampome.org are quotes from scientific texts.  
		...
		if($known_unknown_flag==1||$known_unknown_flag==-1){
				...
				$query_to_get_conn_evidence = "SELECT cr.ConnFragment_id,cd.Type1_id,cd.Type2_id,cd.connection_status,cd.connection_location,
									...
									echo "<br>".$query_to_get_conn_evidence."<br>";
				*/
				$query_to_get_conn_evidence = "SELECT cr.ConnFragment_id,cd.Type1_id,cd.Type2_id,cd.connection_status,cd.connection_location,
									cf.original_id,cf.dt,cf.quote,cf.page_location,cf.pmid_isbn ,a.id,a.title,a.publication,a.year,a.pmid_isbn,
									a.first_page,a.last_page,a.pmcid,a.nihmsid,a.doi,a.open_access,a.citation_count,a.volume,a.issue
									...
									AND ef.Evidence_id=ae.Evidence_id
									AND ae.Article_id=a.id
									AND cd.Type1_id=".$conn_id_neuron[0]." AND cd.Type2_id=".$conn_id_neuron[1];
									#echo "<br>".$query_to_get_conn_evidence."<br>";
...
// check for evidence being open
		...
		if ($_REQUEST['show_1']) //  ==> ON
		{
			...
			//echo $query;
			$rs2 = mysqli_query($GLOBALS['conn'],$query);	
			$query = "SELECT show1 FROM `$name_temporary_table` WHERE title = '$title_paper'";
			//echo '<br>'.$query;
			$rs3 = mysqli_query($GLOBALS['conn'],$query);	
			//$result1 = mysqli_fetch_row($rs3)[0];
			//echo "q r: ".$result1;
			/*while(list($show) = mysqli_fetch_row($rs3))	{		
					echo "q r: ".$show;
				}*/
		}
			...
			$rs2 = mysqli_query($GLOBALS['conn'],$query);	
				...
				<table width="80%" border="0" cellspacing="2" cellpadding="0">
					<tr>			
						...
						<?php print ("<input type='hidden' name='page' value='$page'>
						<input type='hidden' name='id1_neuron' value='$id1_neuron'>
						<input type='hidden' name='val1_property' value='$val1_property'>
						<input type='hidden' name='id2_neuron' value='$id2_neuron'>
						<input type='hidden' name='val2_property' value='$val2_property'>
						<input type='hidden' name='connection_type' value='$connection_type'>
						<input type='hidden' name='known_conn_flag' value='$known_conn_flag'>
						<input type='hidden' name='axonic_basket_flag' value='$axonic_basket_flag'>"); 
						?>
						<?php print ("<input type='hidden' name='conn_name_temporary_table' value='$conn_name_temporary_table'>");?>
						<?php print ("<input type='hidden' name='name_temporary_table' value='$name_temporary_table'>");?>
					...
					<?php 
						...
						?>			
							...
							<form action="property_page_synpro_nm.php" method="post" style="display:inline">
								...
								<input type='hidden' name='axonic_basket_flag' value='$axonic_basket_flag'>"); 
								?>
								<?php //print ("<input type='hidden' name='show1' value='$show1'>"); ?>
								<?php print ("<input type='hidden' name='conn_name_temporary_table' value='$conn_name_temporary_table'>");?>
								<?php print ("<input type='hidden' name='name_temporary_table' value='name_temporary_table'>");?>
								</form>	
							</td>
			...
			<?php	
				...
				{	
					...
					// TABLE OF THE ARTICLES: 
						...
						$query = "SELECT distinct type FROM `$name_temporary_table` WHERE title = '$title_temp[$i]' $subquery ORDER BY FIELD (type,'Known','Axons','Dendrites','Somata') ";
						//echo $query;	
						$rs = mysqli_query($GLOBALS['conn'],$query);	
						...
						if ($show1 == 0)
						{
							...
							print ("<input type='submit' name='show_evi' value=' ' class='show1' title='Show Evidence' alt='Show Evidence'>");
							print ("<input type='hidden' name='show_1' value='1' />");
							print ("<input type='hidden' name='show_0' value='0' />");
							...
							<input type='hidden' name='known_conn_flag' value='$known_conn_flag'>
							<input type='hidden' name='axonic_basket_flag' value='$axonic_basket_flag'>"); 
							print ("<input type='hidden' name='conn_name_temporary_table' value='$conn_name_temporary_table'>");
							print ("<input type='hidden' name='name_temporary_table' value='$name_temporary_table'>");
							print ("</form>");
						}
						if ($show1 == 1)
						{
							print ("<form action='property_page_synpro_nm.php' method='post' style='display:inline' title='Close Evidence' alt='Close Evidence'>");
							print ("<input type='submit' name='show_evi' value=' ' class='show0'>");
							print ("<input type='hidden' name='show_0' value='1' />");
							print ("<input type='hidden' name='show_1' value='0' />");
							print ("<input type='hidden' name='start' value='$page_in' />");
							print ("<input type='hidden' name='stop' value='$page_end' />");
							print ("<input type='hidden' name='title' value='$title_temp[$i]'>");
							print ("<input type='hidden' name='name_show_only' value='$name_show_only'>");
						...
						try {
							...
							$query = "SELECT distinct id_fragment, id_original,id_neuron, quote, page_location, type FROM `$name_temporary_table` WHERE title = '$title_temp[$i]' $subquery ORDER BY FIELD (type,'Known','Axons','Dendrites','Somata') ";	
							//echo $query;
							$rs = mysqli_query($GLOBALS['conn'],$query);	
