...
class epdata
{
	...
	public function retrive_all_information($id)
    {
		...
		$query = "SELECT raw, value1, value2, error, std_sem, n, istim, time FROM $table WHERE id = '$id'";
		$rs = mysql_query($query);
		while(list($raw, $value1, $value2, $error, $std_sem, $n, $istim, $time) = mysql_fetch_row($rs))
		{	
			$this->setRaw($raw);	
