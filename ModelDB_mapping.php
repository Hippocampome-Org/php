		...
		<table width="80%" border="0" cellspacing="2" cellpadding="0">
			...
			<tr>
				...
				<td align="left" width="50%" class="table_neuron_page2">
					<?php $type -> retrive_by_id($id); print $type->getName(); ?> 
				</td>	
				<td align="center" width="30%" class="table_neuron_page2">
					<?php
						print $type->retrieve_supertype($id);
					?> 
				</td>				
			</tr>			
		</table>
...
*/
	...
	<!-- phases section -->
      ...
      <?php 
      	...
      	if ($theta_val != '') {
	      	...
	      	echo "<tr><td class=\"table_neuron_page2\">Theta (deg):</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">Theta range:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">Theta measurements:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">Theta representative selection:&nbsp;&nbsp;&nbsp;</td><td class=\"table_neuron_page2\">no current values</td></tr>";
      	}
      	if ($swr_val != '') {
	      	...
	      	echo "<tr><td class=\"table_neuron_page2\">SWR ratio:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">SWR ratio range:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">SWR ratio measurements:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">SWR ratio representative selection:&nbsp;&nbsp;&nbsp;</td><td class=\"table_neuron_page2\">no current values</td></tr>";
      	}
      	if ($firingrate_val != '') {
	      	...
	      	echo "<tr><td class=\"table_neuron_page2\">In vivo firing rate (Hz):</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">Firing rate range:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">Firing rate measurements:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">Firing rate representative selection:&nbsp;&nbsp;&nbsp;</td><td class=\"table_neuron_page2\">no current values</td></tr>";
      	}
      	if ($other_val != '') {
	      	...
	      	echo "<tr><td class=\"table_neuron_page2\">Other:</td><td class=\"table_neuron_page2\">no current values</td></tr>";
	      	echo "<tr><td class=\"table_neuron_page2\">Other representative selection:&nbsp;&nbsp;&nbsp;</td><td class=\"table_neuron_page2\">no current values</td></tr>";
      	}
      	echo "</table>";
      	echo "<br>";
      ?>

	<!-- TABLE Neuron type census: ******************************************************************************************************************** -->
	<table width="80%" border="0" cellspacing="2" cellpadding="0">
		<tr>
			<td width="20%" align="center" class="table_neuron_page3">
				Neuron Type Census (<a href='Help_Counts_Variable_Definitions.php'>variable definitions</a>)
			</td>			
		</tr>			
	</table>		
	<table width="80%" border="0" cellspacing="2" cellpadding="0">
	<?php
		print ("<tr><td width='20%' align='right'></td>");
		print ("<td align='left' width='80%' class='table_neuron_page2'>");
		print ("<strong>Key:</strong> optimum, [lower bound, upper bound]");
		print ("</td></tr>");
		print ("<tr><td width='20%' align='right'></td>");
		print ("<td align='left' width='80%' bgcolor='#FFFFFF'>");
		print ("<BR>");
		print ("</td></tr>");
		$count = $counts -> retrieve_counts($id);
		print ("<tr><td width='20%' align='right'></td>");
		print ("<td align='left' width='80%' class='table_neuron_page2'>");
		print ("<strong>Counts:</strong> <a href='property_page_counts.php?id_neuron=$id&val_property=counts'>".$counts->retrieve_counts($id)." [".$counts->retrieve_lower_bound($id).", ".$counts->retrieve_upper_bound($id)."] </a>");
		print ("</td></tr>");
		print ("<tr><td width='20%' align='right'></td>");
		print ("<td align='left' width='80%' bgcolor='#FFFFFF'>");
		print ("<BR>");
		print ("</td></tr>");
		print ("</table>");
		$counts -> retrieve_variables($id);
		$n_variables = $counts -> getN_variables();
		$variables = "";
		for ($i1=0; $i1<$n_variables; $i1++)
		{
			$Variable = $counts -> getVariable($i1);
			$Cell_type = $counts -> getCell_type($i1);
			$variables = $variables.$Variable."=".$Cell_type;
			if (($i1 < ($n_variables-1)) && ($n_variables > 1))
			{
				$variables = $variables."; ";
			}
		}
/*
		...
		<table width='80%' border='0' cellspacing='2' cellpadding='0'>
			<tr>
				<td width='20%' align='right' class='table_neuron_page1'>Measurement Equations</td>
				<td align='center' width='40%' class='table_neuron_page1'>".$variables."</td>
				<td align='center' width='40%' class='table_neuron_page1'>Interpretations</td>
			</tr>
		</table>
		<table width='80%' border='0' cellspacing='2' cellpadding='0'>
		");
		$counts -> retrieve_measurement_equation($id);
		$n_eqn = $counts -> getN_measurement_equations();
		for ($i1=0; $i1<$n_eqn; $i1++)
		{
			$Measurement_equation = $counts -> getMeasurement_eqn($i1);
			$Interpretation = $counts -> getInterpretation($i1);
			print ("
				<tr>
					<td width='20%' align='right'></td>
					<td align='left' width='40%' class='table_neuron_page2'>$Measurement_equation</td>
					<td align='center' width='40%' class='table_neuron_page2'>$Interpretation</td>
				</tr>
			");
		}
*/
