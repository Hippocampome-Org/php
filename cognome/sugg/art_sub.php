...
<body>
  ...
  function record_username($conn, $tbl, $username) {
    /*
  ...
  function add_property($tbl,$row_name,$add_prop,$conn) {
    ...
    */
    ...
    $result = $conn->query($sql);
    record_username($conn, $tbl, $_SESSION['user_login']);
  ...
  if($result->num_rows == 0) { 
    ...
    if($result->num_rows == 0 && $_POST['citation'] != '') {
      ...
      $result = $conn->query($sql);
      record_username($conn, 'articles', $_SESSION['user_login']);
      ...
      if ($_POST['subjects']!='') {
        ...
        for ($i=0; $i<count($subjects); $i++)
        {
          ...
          $result = $conn->query($sql);
          record_username($conn, 'article_has_subject', $_SESSION['user_login']);
        }    
      }
      if ($_POST['details']!='') {
        ...
        $result = $conn->query($sql);
        record_username($conn, 'article_has_detail', $_SESSION['user_login']);
      }
      if ($_POST['implmnts']!='') {
        ...
        $result = $conn->query($sql);
        record_username($conn, 'article_has_implmnt', $_SESSION['user_login']);
      }
      if ($_POST['theories']!='') {
        ...
        for ($i=0; $i<count($theories); $i++)
        {
          ...
          $result = $conn->query($sql);
          record_username($conn, 'article_has_theory', $_SESSION['user_login']);
        }    
      }
      if ($_POST['keywords']!='') {
        ...
        for ($i=0; $i<count($keywords); $i++)
        {
          ...
          $result = $conn->query($sql);
          record_username($conn, 'article_has_keyword', $_SESSION['user_login']);
        }    
      }    
    } 
    else {
