<?php
  function get_existing_text($conn, $tbl, $col, $art_mod_id) {
    $existing_entry = '';
  ...
  function display_evidence($conn, $prop_desc, $evid_type, $prop_id, $height, $art_mod_id) {
    $existing_entry = '';
    if ($prop_desc == "Subject") {
      if ($evid_type == "Location") {
        $existing_entry = get_existing_text($conn, "evidence_of_subjects", "evidence_position", $art_mod_id);
      }
      else if ($evid_type=='Description') {
        $existing_entry = get_existing_text($conn, "evidence_of_subjects", "evidence_description", $art_mod_id);
      }
    }   
    else if ($prop_desc == "Detail") {
      if ($evid_type == "Location") {
        $existing_entry = get_existing_text($conn, "evidence_of_details", "evidence_position", $art_mod_id);
      }
      else if ($evid_type=='Description') {
        $existing_entry = get_existing_text($conn, "evidence_of_details", "evidence_description", $art_mod_id);
      }
    }    
    else if ($prop_desc == "Scale") {
      if ($evid_type == "Location") {
        $existing_entry = get_existing_text($conn, "evidence_of_scales", "evidence_position", $art_mod_id);
      }
      else if ($evid_type=='Description') {
        $existing_entry = get_existing_text($conn, "evidence_of_scales", "evidence_description", $art_mod_id);
      }
    }    
    else if ($prop_desc == "Implementation") {
      if ($evid_type == "Location") {
        $existing_entry = get_existing_text($conn, "evidence_of_implmnts", "evidence_position", $art_mod_id);
      }
      else if ($evid_type=='Description') {
        $existing_entry = get_existing_text($conn, "evidence_of_implmnts", "evidence_description", $art_mod_id);
      }
    }    
    else if ($prop_desc == "Region") {
      if ($evid_type == "Location") {
        $existing_entry = get_existing_text($conn, "evidence_of_regions", "evidence_position", $art_mod_id);
      }
      else if ($evid_type=='Description') {
      ...
      if ($evid_type == "Location") {
        ...
        $existing_entry = get_existing_text($conn, "evidence_of_keywords", "evidence_description", $art_mod_id);
      }
    }
